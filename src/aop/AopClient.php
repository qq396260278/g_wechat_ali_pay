...
class AopClient {
	...
	//加密XML节点名称
	private $ENCRYPT_XML_NODE_NAME = "response_alipayEncrypted";
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
			...
			// 执行加密
			$enCryptContent = alipayEncrypt($apiParams['biz_content'], $this->alipayEncryptKey);
...
/** 
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function alipayEncryptAndSign($bizContent, $rsaPublicKeyPem, $rsaPrivateKeyPem, $charset, $isEncrypt, $isSign, $signType='RSA') {
		// 加密，并签名
		if ($isEncrypt && $isSign) {
			$alipayEncrypted = $this->rsaEncrypt($bizContent, $rsaPublicKeyPem, $charset);
			$sign = $this->sign($alipayEncrypted, $signType);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipay><response>$alipayEncrypted</response><alipayEncryption_type>RSA</alipayEncryption_type><sign>$sign</sign><sign_type>$signType</sign_type></alipay>";
			return $response;
		}
		// 加密，不签名
		if ($isEncrypt && (!$isSign)) {
			$alipayEncrypted = $this->rsaEncrypt($bizContent, $rsaPublicKeyPem, $charset);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipay><response>$alipayEncrypted</response><alipayEncryption_type>$signType</alipayEncryption_type></alipay>";
			return $response;
		}
		// 不加密，但签名
		if ((!$isEncrypt) && $isSign) {
	...
	public function rsaDecrypt($data, $rsaPrivateKeyPem, $charset) {
		...
		foreach ($decodes as $n => $decode) {
			if (!openssl_private_alipayDecrypt($decode, $dcyCont, $res)) {
				echo "<br/>" . openssl_error_string() . "<br/>";
			}
			$strnull .= $dcyCont;
		}
	...
	private function parserEncryptJSONItem($responseContent, $nodeName, $nodeIndex) {
		...
		$alipayEncryptParseItem = new EncryptParseItem();
